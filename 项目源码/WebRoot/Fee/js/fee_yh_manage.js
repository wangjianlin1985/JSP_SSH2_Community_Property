...
$(function () { 
	...
	$("#fee_manage").datagrid({
		url : 'Fee/Fee_ajaxQueryFee.action',
...
function initFeeManageTool() {
	fee_manage_tool = {
		init: function() {
			$.ajax({
				url : "FeeType/FeeType_ajaxQueryAllFeeType.action",
				type : "post",
		...
		exportExcel: function() {
			$("#feeQueryForm").form({
			    url:"Fee/Fee_queryFeeOutputToExcel.action",
			});
			//提交表单
			$("#feeQueryForm").submit();
		},
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url : "Fee/Fee_ajaxDeleteFee.action",
							data : {
								feeIds : feeIds.join(","),
							},
							beforeSend : function () {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "Fee/Fee_ajaxModifyFeeQuery.action",
					type : "post",
					data : {
						feeId : rows[0].feeId,
					},
