...
$(function () { 
	...
	$("#manager_manage").datagrid({
		url : 'Manager/Manager_ajaxQueryManager.action',
		fit : true,
		fitColumns : true,
		striped : true,
		rownumbers : true,
				...
				if ($("#managerEditForm").form("validate")) {
					...
					if(!$("#managerEditForm").form("validate")) {
						...
						$("#managerEditForm").form({
						    url:"Manager/Manager_ajaxModifyManager.action",
						    onSubmit: function(){
								if($("#managerEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	manager_manage_tool = {
		...
		exportExcel: function() {
			$("#managerQueryForm").form({
			    url:"Manager/Manager_queryManagerOutputToExcel.action",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url : "Manager/Manager_ajaxDeleteManager.action",
							data : {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "Manager/Manager_ajaxModifyManagerQuery.action",
					type : "post",
					data : {
