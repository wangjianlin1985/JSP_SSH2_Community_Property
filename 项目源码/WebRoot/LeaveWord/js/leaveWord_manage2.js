...
$(function () {
	...
	$("#leaveWord_manage").datagrid({
		url : 'LeaveWord/LeaveWord_ajaxQueryLeaveWord.action',
		fit : true,
		fitColumns : true,
		striped : true,
		rownumbers : true,
				...
				if ($("#leaveWordEditForm").form("validate")) {
					...
					if(!$("#leaveWordEditForm").form("validate")) {
						...
						$("#leaveWordEditForm").form({
						    url:"LeaveWord/LeaveWord_ajaxModifyLeaveWord.action",
						    onSubmit: function(){
								if($("#leaveWordEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
...
function initLeaveWordManageTool() {
	leaveWord_manage_tool = {
		init: function() {
			$.ajax({
				url : "Owner/Owner_ajaxQueryAllOwner.action",
				type : "post",
		...
		exportExcel: function() {
			$("#leaveWordQueryForm").form({
			    url:"LeaveWord/LeaveWord_queryLeaveWordOutputToExcel.action",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url : "LeaveWord/LeaveWord_ajaxDeleteLeaveWord.action",
							data : {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				if(rows[0].replyTime != "--") {
					$.messager.alert("警告操作！", "此投诉已被回复处理！", "warning");
					return; 
				} 
				$.ajax({
					url : "LeaveWord/LeaveWord_ajaxModifyLeaveWordQuery.action",
					type : "post",
					data : {
