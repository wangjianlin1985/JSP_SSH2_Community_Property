...
$(function () { 
	...
	$("#owner_manage").datagrid({
		url : 'Owner/Owner_ajaxQueryOwner.action',
		fit : true,
		fitColumns : true,
		striped : true,
		rownumbers : true,
			...
			handler : function () {
				if ($("#ownerEditForm").form("validate")) {
					...
					if(!$("#ownerEditForm").form("validate")) {
						...
						$("#ownerEditForm").form({
						    url:"Owner/Owner_ajaxModifyOwner.action",
						    onSubmit: function(){
								if($("#ownerEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
...
function initOwnerManageTool() {
	owner_manage_tool = {
		init: function() {
			$.ajax({
				url : "Building/Building_ajaxQueryAllBuilding.action",
				type : "post",
		...
		exportExcel: function() {
			$("#ownerQueryForm").form({
			    url:"Owner/Owner_queryOwnerOutputToExcel.action",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url : "Owner/Owner_ajaxDeleteOwner.action",
							data : {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "Owner/Owner_ajaxModifyOwnerQuery.action",
					type : "post",
					data : {
