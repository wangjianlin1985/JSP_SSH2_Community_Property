...
$(function () { 
	...
	$("#facility_manage").datagrid({
		url : 'Facility/Facility_ajaxQueryFacility.action',
		fit : true,
		fitColumns : true,
		striped : true,
		rownumbers : true,
				...
				if ($("#facilityEditForm").form("validate")) {
					...
					if(!$("#facilityEditForm").form("validate")) {
						...
						$("#facilityEditForm").form({
						    url:"Facility/Facility_ajaxModifyFacility.action",
						    onSubmit: function(){
								if($("#facilityEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	facility_manage_tool = {
		...
		exportExcel: function() {
			$("#facilityQueryForm").form({
			    url:"Facility/Facility_queryFacilityOutputToExcel.action",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url : "Facility/Facility_ajaxDeleteFacility.action",
							data : {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "Facility/Facility_ajaxModifyFacilityQuery.action",
					type : "post",
					data : {
