...
$(function () { 
	...
	$("#salary_manage").datagrid({
		url : 'Salary/Salary_ajaxQuerySalary.action',
		fit : true,
		fitColumns : true,
		striped : true,
		rownumbers : true,
			...
			handler : function () {
				if ($("#salaryEditForm").form("validate")) {
					...
					if(!$("#salaryEditForm").form("validate")) {
						...
						$("#salaryEditForm").form({
						    url:"Salary/Salary_ajaxModifySalary.action",
						    onSubmit: function(){
								if($("#salaryEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	salary_manage_tool = {
		...
		exportExcel: function() {
			$("#salaryQueryForm").form({
			    url:"Salary/Salary_querySalaryOutputToExcel.action",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url : "Salary/Salary_ajaxDeleteSalary.action",
							data : {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "Salary/Salary_ajaxModifySalaryQuery.action",
					type : "post",
					data : {
