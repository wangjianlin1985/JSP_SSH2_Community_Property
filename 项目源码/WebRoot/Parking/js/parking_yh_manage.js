...
$(function () { 
	...
	$("#parking_manage").datagrid({
		url : 'Parking/Parking_ajaxQueryParking.action',
	...
	parking_manage_tool = {
		...
		exportExcel: function() {
			$("#parkingQueryForm").form({
			    url:"Parking/Parking_queryParkingOutputToExcel.action",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url : "Parking/Parking_ajaxDeleteParking.action",
							data : {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "Parking/Parking_ajaxModifyParkingQuery.action",
					type : "post",
					data : {
