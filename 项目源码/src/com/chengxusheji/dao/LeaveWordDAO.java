...
public class LeaveWordDAO {
    ...
    public ArrayList<LeaveWord> QueryLeaveWordInfo(String title,String addTime,Owner ownerObj,String opUser,int currentPage) { 
    	Session s = factory.getCurrentSession();
    	...
    	if(null != ownerObj && ownerObj.getOwnerId()!=0) hql += " and leaveWord.ownerObj.ownerId=" + ownerObj.getOwnerId();
    	if(!opUser.equals("")) hql = hql + " and leaveWord.opUser like '%" + opUser + "%'";
    ...
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public ArrayList<LeaveWord> QueryLeaveWordInfo(String title,String addTime,Owner ownerObj,String opUser) { 
    	Session s = factory.getCurrentSession();
    	String hql = "From LeaveWord leaveWord where 1=1";
    	if(!title.equals("")) hql = hql + " and leaveWord.title like '%" + title + "%'";
    	if(!addTime.equals("")) hql = hql + " and leaveWord.addTime like '%" + addTime + "%'";
    ...
    /*计算总的页数和记录数*/
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public void CalculateTotalPageAndRecordNumber(String title,String addTime,Owner ownerObj,String opUser) {
        Session s = factory.getCurrentSession();
        String hql = "From LeaveWord leaveWord where 1=1";
        if(!title.equals("")) hql = hql + " and leaveWord.title like '%" + title + "%'";
        if(!addTime.equals("")) hql = hql + " and leaveWord.addTime like '%" + addTime + "%'";
