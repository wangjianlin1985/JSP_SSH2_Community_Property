...
public class LeaveWordAction extends ActionSupport {
    ...
    public void ajaxAddLeaveWord() throws IOException, JSONException {
        ...
        try {
            Owner ownerObj = ownerDAO.GetOwnerByOwnerId(leaveWord.getOwnerObj().getOwnerId());
            leaveWord.setOwnerObj(ownerObj);
    ...
    public void ajaxQueryLeaveWord() throws IOException, JSONException {
        ...
        List<LeaveWord> leaveWordList = leaveWordDAO.QueryLeaveWordInfo(title, addTime, ownerObj, opUser, page);
        /*计算总的页数和总的记录数*/
        leaveWordDAO.CalculateTotalPageAndRecordNumber(title, addTime, ownerObj, opUser);
        /*获取到总的页码数目*/
    ...
    public String queryLeaveWordOutputToExcel() { 
        ...
        if(addTime == null) addTime = "";
        if(opUser == null) opUser = "";
        List<LeaveWord> leaveWordList = leaveWordDAO.QueryLeaveWordInfo(title,addTime,ownerObj,opUser);
        ...
        for(int i=0;i<leaveWordList.size();i++) {
        	...
        	dataset.add(new String[]{leaveWord.getLeaveWordId() + "",leaveWord.getTitle(),leaveWord.getContent(),leaveWord.getAddTime(),leaveWord.getOwnerObj().getOwnerName(),
leaveWord.getReplyContent(),leaveWord.getReplyTime(),leaveWord.getOpUser()});
        }
        /*
